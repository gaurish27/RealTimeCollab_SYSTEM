...
const Page = () => {
    ...
    const handleLounchWorkspace = async () => {
        setLoading(true);
        try {
            const projectId = pathName.slice(0, pathName.length-10).split("/").pop();
            const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/projects/workspace/start/${projectId}`, {}, {
                headers: {
                    Authorization: `Bearer ${getToken()}`
                }
            });
            if (response.data.port) {
                router.push(`http://localhost:${response.data.port}/?folder=/home/workspace`);
            } else {
                setLoading(false);
                alert("Failed to start workspace. Port not returned.");
            }
        } catch (error) {
